...
def trainAndGenerateReport(X_train, X_test, y_train, y_test, clf, model_name):
    ...
    print(metrics.classification_report(y_test, y_pred, target_names=['Wait','Buy']))
...
clf = trainAndGenerateReport(X_train, X_test, y_train, y_test, clf, model_name)
save_model(clf, model_name)

# Random Forest Classifier
model_name = 'random_forest_classifier'
clf = RandomForestClassifier(max_depth=10, random_state=random_state, n_estimators=100)
clf = make_pipeline(StandardScaler(), clf, verbose=True)
...
clf = MLPClassifier(hidden_layer_sizes=[64, 32, 16], learning_rate_init=0.01, random_state=random_state, max_iter=500)
clf = make_pipeline(StandardScaler(), clf, verbose=True)
